...
const long timeOutPeriod = 10000;
...
void validation(String ID){
  ...
  else if(reply.substring(0,2)  == "OK"){
...
void lock(String ID){
  ...
  index2 = reply.indexOf(" ", index1 + 1);
  if ((reply.substring(0,2)  == "OK") && (reply.charAt(index2+1) == '1')){
    Serial.println("Successfull");
  }
  else{
    Serial.println("Bad Commands");
...
void getStatus(){
  String message = "GET " + lockerAddress + " STAT " + "000000000000";
  sendRequest(message);
  String reply = getResponse(timeOutPeriod);
  index1 = reply.indexOf(" ");
  index2 = reply.indexOf(" ", index1 + 1);
  if ((reply.substring(0,2)  == "OK") && (reply.charAt(index2+1) == '0')){
    Serial.println("Locker is unoccupied");
  }
  else if ((reply.substring(0,2)  == "OK") && (reply.charAt(index2+1) == '1')){
    Serial.println("Locker is occupied");
  }
}
